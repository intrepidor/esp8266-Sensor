...
bool GenericSensor::acquire_setup(void) {
	...
	else if (getModule() == sensorModule::digital || getModule() == sensorModule::analog_digital) {
		pinMode(digital_pin, INPUT);
		started = true;
	}
	else if (getModule() == sensorModule::taskclock || getModule() == sensorModule::Sharp_GP2Y10_DustSensor) {
...
bool GenericSensor::acquire1(void) {
	if (started) {
		...
		////////////////////////////////////////////////////////////////////////////////////////////
		if (getModule() == sensorModule::analog || getModule() == sensorModule::analog_digital) {
			last_reading_timestamp_ms = now_ms;
			reading_count++;
		...
		////////////////////////////////////////////////////////////////////////////////////////////
		else if (getModule() == sensorModule::Sharp_GP2Y10_DustSensor) {
			static unsigned long last_sample_timestamp_ms = millis();
			static const unsigned int GP2Y10_MIN_SAMPLE_PERIOD_MS = 10; // Take measurements not faster than every 0.1 seconds
			static const unsigned int GP2Y10_SAMPLES_PER_CYCLE = 400; // takes ~10 seconds average
			static const unsigned int GP2Y10_MAX_CYCLE_PERIOD_MS = 20 * GP2Y10_MIN_SAMPLE_PERIOD_MS
					* GP2Y10_SAMPLES_PER_CYCLE; // all samples must complete in a minimum time, else start cycle over
			static int print_column = 0;
			 ...
			 */
			if (now_ms > (last_reading_timestamp_ms + GP2Y10_MAX_CYCLE_PERIOD_MS)) {
			...
			DEBUGPRINT(DebugLevel::SHARPGP2Y10, String(a));
			...
			if (reading_count_within_cycle > GP2Y10_SAMPLES_PER_CYCLE) {
				...
				generic_accumulator = 0;
				reading_count_within_cycle = 0;
				last_reading_timestamp_ms = now_ms; // timestamp for this long cycle reading
				...
				DEBUGPRINTLN(DebugLevel::SHARPGP2Y10, "");
				DEBUGPRINT(DebugLevel::SHARPGP2Y10, "#### Average: " + String(a0));
				...
				// Calibration corrected values
				setAnalog(static_cast<float>(a1));
				DEBUGPRINT(DebugLevel::SHARPGP2Y10, "\t Density: ug/m^3=" + String(a1));
				unsigned long time_now = millis();
				DEBUGPRINTLN(DebugLevel::SHARPGP2Y10,
						"\t Period[ms]: " + String(time_now - last_sample_timestamp_ms));
				last_sample_timestamp_ms = time_now;
				print_column = 0;
			}
			if ((print_column++ % 6) == 0) {
				DEBUGPRINTLN(DebugLevel::SHARPGP2Y10, "");
		...
		////////////////////////////////////////////////////////////////////////////////////////////
		else if (getModule() == sensorModule::taskclock) {
		}
	}
	return true;
}
